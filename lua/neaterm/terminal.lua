...
function Neaterm:setup_keymaps()
	...
	local maps = {
		...
		{
			key = self.opts.keymaps.repl_inspector,
			func = function()
				self:create_variable_inspector()
			end,
		...
		{
			key = self.opts.keymaps.plot_viewer,
			func = function()
		...
		{
			key = self.opts.keymaps.doc_viewer,
			func = function()
				self:setup_doc_viewer()
			end,
			desc = "doc_viewer",
			mode = { "n" },
		},
...
function Neaterm:move_terminal(direction)
	...
	if not term or not term.window then
		return
	end
...
function Neaterm:create_variable_inspector()
	...
	local win_opts = {
		...
		row = 1,
		col = self.opts.repl.inspect_position == "left" and 1
			or (vim.o.columns - math.floor(vim.o.columns * (self.opts.repl.inspect_width or 0.2)) - 1),
	...
	self.var_inspector.update_timer:start(
		1000,
		1000,
...
function Neaterm:update_variable_inspector()
	if not self.current_repl or not self.var_inspector then
		return
	end
	...
	-- Send the command to get variables and capture the output
	local cmd = config.get_variables_cmd
	local output = ""

	-- Use a job to run the command silently
	vim.fn.jobstart(cmd, {
		...
		on_exit = function()
			...
			for _, var in ipairs(vars) do
				...
				table.insert(lines, string.format("| %s | %s | %s |", name, type, info))
			end
...
function Neaterm:setup_repl_completion()
	...
	local completion_sources = {
		...
		r = {
			get_completions_cmd = "utils:::.get_completions()",
			trigger_chars = { "$", ":", "@" },
		},
		julia = {
			get_completions_cmd = "using REPL; REPL.REPLCompletions.completions()",
			trigger_chars = { ".", "_" },
		},
	}

	-- Setup completion
	vim.bo[self.current_repl.buf].omnifunc = function(findstart, base)
		if findstart == 1 then
		...
		local completions = self:get_repl_completions(source.get_completions_cmd)
		return vim.tbl_filter(function(item)
			return item.word:match("^" .. base)
		end, completions)
	end
end

-- 2. Interactive Plot Viewer
function Neaterm:setup_plot_viewer()
	-- Create plot viewer window
	self.plot_viewer = {
...
def save_plot():
    plt.savefig('/tmp/neaterm_plot.png')
    plt.close()
      ]],
			save_cmd = "save_plot()",
		},
...
pdf('/tmp/neaterm_plot.pdf')
	...
	vim.loop.new_fs_event():start("/tmp", {}, function(err, filename)
		if filename and (filename:match("neaterm_plot%.png$") or filename:match("neaterm_plot%.pdf$")) then
			self:update_plot_viewer(filename)
		end
	end)
end

-- 3. Debug Integration
function Neaterm:setup_debug_integration()
	self.debug = {
		active = false,
		breakpoints = {},
		current_frame = nil,
	...
	-- Add debug commands
	for cmd, func in pairs(debug_commands[self.current_repl.filetype] or {}) do
		self["debug_" .. cmd] = function()
			if self.current_repl then
				self:send_text(func)
			end
		end
...
function Neaterm:setup_workspace_management()
	self.workspace = {
		variables = {},
		history = {},
		plots = {},
	...
	local workspace_commands = {
		python = {
			save = "import dill; dill.dump_session('workspace.pkl')",
			load = "import dill; dill.load_session('workspace.pkl')",
			clear = "import sys; sys.modules[__name__].__dict__.clear()",
		},
		r = {
			save = "save.image('workspace.RData')",
			load = "load('workspace.RData')",
			clear = "rm(list=ls())",
		},
	}

	-- Add workspace methods
	self.save_workspace = function()
		if self.current_repl and workspace_commands[self.current_repl.filetype] then
			self:send_text(workspace_commands[self.current_repl.filetype].save)
		end
	end

	self.load_workspace = function()
		if self.current_repl and workspace_commands[self.current_repl.filetype] then
			self:send_text(workspace_commands[self.current_repl.filetype].load)
		end
	end
end

-- 5. Package/Module Manager
function Neaterm:setup_package_manager()
	...
	function self:show_package_manager()
		...
		-- Create package manager window
		require("fzf-lua").fzf_exec(cmd.list, {
			prompt = "Packages > ",
			actions = {
				["default"] = function(selected)
				...
				["ctrl-i"] = function(selected)
					-- Install package
					self:send_text(cmd.install .. " " .. selected[1])
				end,
				["ctrl-x"] = function(selected)
					-- Uninstall package
					self:send_text(cmd.uninstall .. " " .. selected[1])
				end,
				["ctrl-u"] = function(selected)
					-- Update package
					self:send_text(cmd.update .. " " .. selected[1])
				end,
			},
		})
	end
...
-- 6. Code Snippets
function Neaterm:setup_snippets()
	self.snippets = {
		python = {
	...
	function self:expand_snippet(trigger)
		...
		if not snippets or not snippets[trigger] then
			return
		end

		local snippet = snippets[trigger]
		-- TODO: Add snippet expansion logic
		self:send_text(snippet)
	end
end

-- 7. Documentation Viewer
function Neaterm:setup_doc_viewer()
	local doc_commands = {
		python = {
			get_doc = "help({object})",
			get_signature = "print({object}.__doc__)",
		},
		r = {
			get_doc = "?{object}",
	...
	function self:show_documentation(object)
		...
		self:send_text(cmd.get_doc:gsub("{object}", object))
	end
end
...
function Neaterm:setup(opts)
	...
	self:setup_plot_viewer()
	self:setup_debug_integration()
	self:setup_workspace_management()
