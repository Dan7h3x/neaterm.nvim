...
function Neaterm:setup_keymaps(keymaps)
  local opts = { noremap = true, silent = true }

  for _, map in pairs(keymaps) do
    if map.enabled then
      vim.keymap.set('n', map.key, function() self:toggle_terminal() end, opts)
    end
  end

  -- Additional keymaps for visual mode, etc.
  if keymaps.repl_send_selection.enabled then
    vim.keymap.set('v', keymaps.repl_send_selection.key, function()
