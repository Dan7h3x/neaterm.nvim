...
function M.create_bar(neaterm)
  ...
  local bar_width = math.min(math.floor(win_width * 0.8), 120) -- Cap at 120 chars or 80% of screen
  local bar_height = 1
  local bar_row = 1
...
function M.update_bar(neaterm)
  ...
  if not neaterm.bar_buf or not api.nvim_buf_is_valid(neaterm.bar_buf) or
     not neaterm.bar_win or not api.nvim_win_is_valid(neaterm.bar_win) then
...
function M.get_terminal_at_position(neaterm, col)
  if not neaterm.terminal_positions then return nil end
